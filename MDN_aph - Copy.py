...
class MDN(nn.Module):
    ...
    def covariance_frobenius_norm(self, sigma):
        batch_size, num_gaussians, output_dim = sigma.size()
        cov_matrix = torch.bmm(sigma.view(batch_size * num_gaussians, output_dim, 1), sigma.view(batch_size * num_gaussians, 1, output_dim))
        frobenius_norm = torch.norm(cov_matrix, dim=[1, 2])
        frobenius_norm = frobenius_norm.view(batch_size, num_gaussians).mean()
        return frobenius_norm

def mdn_loss(pi, sigma, mu, y, model, cov_reg=1e-3):
    ...
    likelihood_loss = -log_sum.mean()

    # 计算协方差矩阵的 Frobenius 范数
    covariance_norm = model.covariance_frobenius_norm(sigma)
    covariance_regularization = cov_reg * covariance_norm

    return likelihood_loss + covariance_regularization

def train_mdn(model, train_dl, epochs=200, lr=1e-3, l2=1e-3, cov_reg=1e-3):
    weight_params = []
    bias_params = []
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_dl:
            ...
            pi, sigma, mu = model(x)
            loss = mdn_loss(pi, sigma, mu, y, model, cov_reg)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def load_real_data(aphy_file_path, rrs_file_path):
    ...
    test_real_dl = DataLoader(test_dataset_real, batch_size=128, shuffle=False, num_workers=12)
...
def plot_results(predictions_rescaled, actuals_rescaled, save_dir, threshold=0.9, mode='test'):
...
if __name__ == '__main__':
    ...
    train_real_dl, test_real_dl, input_dim, output_dim, min_val, max_val  = load_real_data('F:\\Geo\\Data\\Real\\aphy_RA_EMIT.csv','F:\\Geo\\Data\\Real\\Rrs_RA_EMIT.csv')
    test_real_Sep, _, _,_,_  = load_real_test('F:\\Geo\\Data\\Real\\aphy_RA_EMIT_Sep.csv','F:\\Geo\\Data\\Real\\Rrs_RA_EMIT_Sep.csv')
    test_real_Oct, _, _,_,_  = load_real_test('F:\\Geo\\Data\\Real\\aphy_RA_EMIT_Oct.csv','F:\\Geo\\Data\\Real\\Rrs_RA_EMIT_Oct.csv')

    save_dir = "F:\\aphy-chla-predictions\\plots\\MDN_aph_EMIT_2"
